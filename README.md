...
| 로컬 | 사람 탐지를 제대로 볼 거면 NPC 에셋 zip 받아서 풀기 (약 500 MB, **저장소에 없음**) | §8 |
...
> **사람은 Play 할 때 `PersonPlacer` 가 자동으로 세운다 — 방마다 넓이에 맞춰
> 0~3명, 모두 23명.** 씬에 손으로 놓인 NPC 3개는 복제 원본으로만 쓰이고 숨겨진다
> (원본은 scale 3 이라 집 기준으로 키 1.08 m 인 아이 크기다 — 아래 참고).
> 그 에셋은 저장소에 없어서 **안 받으면 세울 원본이 없고 YOLO 가 잡을 대상도
> 없다.** 탐지를 보려면 아래 zip 이 사실상 필수다.
>
> 자리는 `simulator/bridge/place_people.py` 가 방 인덱스에서 미리 뽑아
> `Assets/Resources/person_spawn_points.json` 에 넣어 뒀다 (추적 대상). 손으로
> 놓을 때 틀리기 쉬운 두 가지를 이쪽이 대신 맞춘다 — **바닥 높이**(1층 바닥이
> world z = −3.06 m 다. 0 이 아니다. 2층이 0, 3층이 3.10) 와 **스케일**(집이
> scale 5 로 들어와 있으므로 실측 1.8 m 프리팹도 5 배여야 키가 맞는다).
> 각 사람은 그 방의 **스캔 지점**(드론이 360° 도는 바로 그 점) 주위 0.9~5 m 에
> 서고 그 점을 바라본다 — 정면이 YOLO 가 제일 잘 잡는 각도다.
>
> ```bash
> # 방을 바꾸거나 인원을 조절했을 때만 다시 뽑는다 (data/final_npy 필요)
> python simulator/bridge/place_people.py --dry-run   # 방별 인원 표만
> python simulator/bridge/place_people.py             # JSON 갱신
> ```
>
> 좁은 방 3곳(5.5~25 m²)은 복셀 그리드상 설 자리가 가구로 차 있어 요청보다 적게
> 선다 — 표에 `자리 부족` 으로 나온다.
>
...
> 명으로 합쳐진다). 세운 오브젝트에 `PersonTarget` 을 붙이면 id 와 콜라이더가
> 자동으로 생기고 그 id 로 걸러진다.
