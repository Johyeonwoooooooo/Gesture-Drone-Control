# 순찰 에이전트 — 외부 디텍터 계약 (UDP 9004)

> **⚠ 이건 더 이상 기본 경로가 아니다.** 360° 스캔과 사람 탐지는 이제 Unity가
> 직접 한다 — `PatrolPersonDetection.cs` 가 자기 카메라 프레임을 TCP 9100의
> YOLO에 넘기고, 결과를 상태 채널(UDP 9002)의 `detect` 이벤트로 올린다.
> 프로토콜은 `README.md` §5, 웹까지 가는 경로는 `API.md`.
>
> 이 문서는 **탐지기를 별도 프로세스로 돌릴 때**의 계약이다. 그 경로는
> `--scan-mode rc` 에서(또는 Unity가 `scan` 을 구현하지 않은 구버전일 때)
> 여전히 유효하고, `patrol/detect_events.py` 가 그대로 받는다.
