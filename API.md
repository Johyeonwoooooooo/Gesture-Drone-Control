...
`mode` 가 `"find"` 면 순찰이 아니라 물체 찾기다("거실 소파로 가줘"). 지금은
`rooms` 가 그 물체가 있는 방으로 채워지니 그대로 순찰 대상으로 써도 되고,
무시해도 된다.
...
> 전부 null 이라 자리는 비어 있다. 별칭 정본을 `room_aliases.json` 하나로 두는
> 게 낫다고 본다.)
